...
export async function createStockMovement(payload: StockMovementInput) {
  ...
  let newQuantity = currentBatch.quantity;

  switch (payload.transaction_type) {
    case "SALE":
    case "DAMAGE":
    case "WASTAGE":
      newQuantity = currentBatch.quantity - payload.quantity;
  ...
  const transactionNotes =
    payload.transaction_type === "ADJUSTMENT"
